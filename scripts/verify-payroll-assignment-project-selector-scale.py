...
if text("VERSION").strip() != "1.0.87":
    fail("VERSION must be 1.0.87")
contract = json.loads(text("merge/payroll-assignment-project-selector-scale.json"))
if contract.get("release") != "1.0.87":
    fail("assignment-project selector contract release must be 1.0.87")
...
print("Verified SESCCO MS 1.0.87 assignment project selector scale safety: embedded search, bounded count-free paging, date-valid active projects, current-project exclusion and no directory-cache save dependency.")
