...
if (contract.get("authority") or {}).get("legacy_role_bridge") is not False:
    fail("1.0.111 must keep the legacy runtime role bridge retired")
...
for marker in ('("rental-supervisor", "Rental Supervisor / Foreman")', 'key="role-rental-supervisor"'):
    if marker not in rental_supervisor:
        fail(f"1.0.111 Rental Supervisor profile migration marker missing: {marker}")
for marker in ("def enforce_profile_authority", 'name="access_profile"', 'name="role"'):
    if marker not in cutover:
        fail(f"1.0.111 authority cutover migration marker missing: {marker}")
...
print("Verified SESCCO MS 1.0.111 granular access authority: 94 permissions including independent Sourcing grants, system profiles, explicit scopes, request cache and profile-only runtime authority.")
