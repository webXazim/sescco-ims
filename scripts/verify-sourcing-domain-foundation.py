...
if contract.get("schema_change") is not True:
    fail("1.0.117 must preserve the Sourcing schema foundation")
if contract.get("module_access_enabled") is not True:
    fail("1.0.117 must enable Sourcing only through the explicit permission cutover")
if contract.get("permission_cutover_contract") != "merge/sourcing-access-control.json":
    fail("Sourcing foundation is not bound to the 1.0.117 permission cutover contract")
...
if "1.0.117" not in text("docs/SOURCING_DOMAIN_FOUNDATION.md"):
    fail("Sourcing foundation guide is not version-bound")

print("PASS: SESCCO MS 1.0.117 independent Sourcing Directory foundation remains operationally isolated and is exposed only through explicit persisted permissions.")
