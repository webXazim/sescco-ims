...
if text("VERSION").strip() != VERSION: fail("VERSION must be 1.0.113")
...
if contract.get("schema_change") is not False: fail("1.0.113 must not introduce a database migration")
if contract.get("payroll_formula_change") is not False: fail("1.0.113 must not change Payroll formulas")
...
if "1.0.113" not in text("docs/CROSS_MODULE_ACCESS_LEAK_CLOSURE.md"): fail("operator/security guide not version-bound")
print("PASS: SESCCO MS 1.0.113 cross-module access leak closure verified across search, profiles, Documents, Reports, Management, Archive/Trash and scoped selectors.")
