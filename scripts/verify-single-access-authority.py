...
if contract.get("schema_change") is not True or contract.get("payroll_formula_change") is not False:
    fail("1.0.112 must declare the Accounts authorization schema change and no Payroll formula change")
...
if "null=True" in access_field or "blank=True" in access_field:
    fail("CompanyMembership.access_profile must be required in 1.0.112")
...
for marker in (
    ...
    if marker not in migration:
        fail(f"1.0.112 migration marker missing: {marker}")
...
print("Verified SESCCO MS 1.0.112 single authorization authority: User.role retired, profile-only runtime decisions, owner/profile protection, Django-admin separation and fail-closed reconciliation.")
