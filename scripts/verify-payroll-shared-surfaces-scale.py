...
if text("VERSION").strip() != "1.0.77":
    fail("VERSION must be 1.0.77")

contract = json.loads(text("merge/payroll-shared-surfaces-scale.json"))
if contract.get("release") != "1.0.77":
    fail("shared-surface contract release must be 1.0.77")
...
if contract.get("schema_change") is not False or contract.get("payroll_formula_change") is not False:
    fail("1.0.77 must not claim a schema or Payroll formula change")
