...
if text("VERSION").strip() != "1.0.87": fail("VERSION must be 1.0.87")
contract=json.loads(text("merge/payroll-drawer-selector-scale.json"))
if contract.get("release") != "1.0.87": fail("drawer selector contract release must be 1.0.87")
...
print("Verified SESCCO MS 1.0.87 cross-workspace drawer selector scale hardening: bounded search-in-dropdown selectors, count-free pages, cancellable requests, financial availability filtering and corrected assignment initializer placement.")
