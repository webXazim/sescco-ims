...
if contract.get("schema_change") is not True:
    fail("1.0.111 must declare the Trade alias-search schema change")
...
if "1.0.111" not in text("docs/SOURCING_TRADE_WORKFORCE_CATALOG.md"):
    fail("Trade/Workforce operator guide is not version-bound")

print("PASS: SESCCO MS 1.0.111 Worker Trade Master & Workforce Catalog verified: controlled aliases, company-scoped trade master, reference workforce quantity/rates, immutable revisions, view/edit separation and zero Rental Payroll/Inventory/Accounting integration.")
