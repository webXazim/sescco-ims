...
from ..models import SourcingEntityStatus, SourcingManpowerSupplier
from .scale import manpower_directory_scale_annotations
...
def manpower_directory_page(*, company, params) -> ManpowerDirectoryResult:
    ...
    qs = manpower_directory_scale_annotations(company=company, queryset=qs, contact_query=query)
    if query:
        qs = qs.filter(
    ...
    return ManpowerDirectoryResult(
        page_obj=page_obj,
        query=query,
        status=status,
        sort=sort,
...
def manpower_history(*, company, supplier, limit: int = 40):
    return list(
