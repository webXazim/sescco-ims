(() => {
    ...
    return `<section class="page ui-v2-payroll-page ui-v2-prs-internal-page ui-v2-prs-internal-employees">
      ...
      <section class="data-panel ui-v2-payroll-panel ui-v2-payroll-register">
        <div class="table-toolbar ui-v2-payroll-register__toolbar ui-v2-payroll-register__toolbar--dimensions">
          ...
          <select id="employeeBranchFilter" class="compact-select ui-v2-select ui-v2-payroll-compact-select" aria-label="Filter by branch">${branchOptions.map(option => `<option ${state.employeeBranch === option ? 'selected' : ''}>${escapeHtml(option)}</option>`).join('')}</select>
          <select id="employeeDepartmentFilter" class="compact-select ui-v2-select ui-v2-payroll-compact-select" aria-label="Filter by department">${departmentOptions.map(option => `<option ${state.employeeDepartment === option ? 'selected' : ''}>${escapeHtml(option)}</option>`).join('')}</select>
          <select id="employeeStatusFilter" class="compact-select ui-v2-select ui-v2-payroll-compact-select" aria-label="Filter by employee status">${['All','Active','On Leave','Inactive','Terminated'].map(option => `<option value="${option}" ${state.employeeStatus === option ? 'selected' : ''}>${option === 'All' ? 'All statuses' : option}</option>`).join('')}</select>
          <select id="employeeWpsFilter" class="compact-select ui-v2-select ui-v2-payroll-compact-select" aria-label="Filter by WPS readiness"><option ${state.employeeWps === 'All' ? 'selected' : ''}>All</option><option ${state.employeeWps === 'WPS ready' ? 'selected' : ''}>WPS ready</option><option ${state.employeeWps === 'Needs setup' ? 'selected' : ''}>Needs setup</option></select>
        ...
        <aside class="detail-stack">
          <section class="detail-card">
            ...
            <dl class="detail-list">
              ...
              <div><dt>Address</dt><dd>${escapeHtml(supplier.address || '—')}</dd></div>
            </dl>
          </section>
          <section class="detail-card">
    ...
    return `
      ...
      <section class="ui-v2-payroll-panel ui-v2-payroll-register ui-v2-payroll-timesheet-workspace ${state.timesheetFullscreen ? 'is-fullscreen' : ''}" aria-label="Internal company attendance workspace">
        <div class="ui-v2-payroll-register__toolbar ui-v2-payroll-timesheet-toolbar">
          ...
          <select id="timesheetBranchFilter" class="ui-v2-select ui-v2-payroll-compact-select" aria-label="Branch / Office">${branchOptions.map(option => `<option ${state.timesheetBranch === option ? 'selected' : ''}>${escapeHtml(option)}</option>`).join('')}</select>
          <select id="timesheetDepartmentFilter" class="ui-v2-select ui-v2-payroll-compact-select" aria-label="Department">${departments.map(option => `<option ${state.timesheetDepartment === option ? 'selected' : ''}>${escapeHtml(option)}</option>`).join('')}</select>
    ...
    return `
      ...
      <section class="ui-v2-payroll-panel ui-v2-payroll-register ui-v2-payroll-timesheet-workspace ui-v2-prs-rental-timesheet-workspace ${state.timesheetFullscreen ? 'is-fullscreen' : ''}" aria-label="Rental project manpower timesheet workspace">
        <div class="ui-v2-payroll-register__toolbar ui-v2-payroll-timesheet-toolbar">
          ...
          <select class="ui-v2-select ui-v2-payroll-compact-select" id="rentalTimesheetProjectFilter" aria-label="Project">${state.projects.filter(project=>project.status !== 'Completed').map(item=>`<option value="${escapeHtml(item.id)}" ${item.id===state.rentalTimesheetProject?'selected':''}>${escapeHtml(item.name)}</option>`).join('')}</select>
          <select class="ui-v2-select ui-v2-payroll-compact-select" id="rentalTimesheetSupplierFilter" aria-label="Supplier"><option>All suppliers</option>${supplierOptions.map(item=>`<option value="${escapeHtml(item.id)}" ${item.id===state.rentalTimesheetSupplier?'selected':''}>${escapeHtml(item.name)}</option>`).join('')}</select>
        ...
        ${state.payrollView === 'review' ? payrollReviewTemplate(run, allRows) : `<div class="payroll-run-layout">
          <section class="data-panel payroll-register">
            <div class="table-toolbar payroll-toolbar">
              ...
              <label class="timesheet-filter"><span>Branch / Office</span><select id="payrollBranchFilter" class="compact-select">${branchOptions.map(option => `<option ${state.payrollBranch === option ? 'selected' : ''}>${escapeHtml(option)}</option>`).join('')}</select></label>
              <label class="timesheet-filter"><span>Department</span><select id="payrollDepartmentFilter" class="compact-select">${departmentOptions.map(option => `<option ${state.payrollDepartment === option ? 'selected' : ''}>${escapeHtml(option)}</option>`).join('')}</select></label>
              <label class="timesheet-filter"><span>Readiness</span><select id="payrollReadinessFilter" class="compact-select"><option ${state.payrollReadiness === 'All' ? 'selected' : ''}>All</option><option ${state.payrollReadiness === 'Ready' ? 'selected' : ''}>Ready</option><option ${state.payrollReadiness === 'Blocked' ? 'selected' : ''}>Blocked</option></select></label>
          ...
          <aside class="payroll-side-stack">
            <section class="data-panel payroll-control-panel">
              ...
              <div class="payroll-input-list">
                <button data-route-link="salary-setup"><span class="payroll-input-icon">S</span><span><strong>Salary structures</strong><small>${totals.ready} ready · ${totals.blocked} blocked</small></span><em>${totals.blocked ? 'Fix' : 'Ready'}</em></button>
                <button data-route-link="timesheets"><span class="payroll-input-icon">T</span><span><strong>Attendance & overtime</strong><small>${escapeHtml(tsStatus)}</small></span><em>${inputReady ? 'Locked' : calculationInputReady ? 'Approved' : 'Open'}</em></button>
                <button data-route-link="adjustments"><span class="payroll-input-icon">A</span><span><strong>Period adjustments</strong><small>${allRows.reduce((n,row)=>n+(row.pendingAdjustments?.length||0),0)} pending · approved items calculate automatically</small></span><em>Review</em></button>
              </div>
            </section>
  ...
  function openRentalWorkerActionDrawer(workerId, action) {
    ...
    if (action === 'transfer') {
      ...
      drawerBody.innerHTML = `<section class="form-section"><div class="form-section__head"><strong>Permanent worker identity</strong><span>Editing master details does not create or rewrite an assignment.</span></div><div class="form-grid">${namedField('Worker ID','rental-action-worker-code',rentalWorkerCode(worker))}${namedField('Full name','rental-action-name',worker.name || '')}${namedField('Iqama / National ID','rental-action-national-id',worker.nationalId || '')}${namedField('Phone','rental-action-phone',worker.phone || '')}${namedSelectFieldValue('Master status','rental-action-master-status',['Active','Inactive'],worker.masterStatus || worker.statusValue || 'Active')}${namedTextareaField('Notes','rental-action-notes',worker.notes || '')}</div></section><section class="source-note">${icon('info')}<span>Supplier ownership is preserved here. Project/trade/rate changes are separate effective-dated assignment events.</span></section>`;
    ...
    return `<section class="page rental-workforce-page">
      ...
      <section class="panel panel--flush rental-directory-panel">
        ...
        <div class="rental-filterbar">
          ...
          <select class="select" id="rentalSupplierFilter"><option value="All suppliers">All suppliers</option>${state.suppliers.filter(s => s.status === 'Active').map(s => `<option value="${escapeHtml(s.id)}" ${state.rentalSupplier === s.id ? 'selected' : ''}>${escapeHtml(s.name)}</option>`).join('')}</select>
  ...
  function rentalSettlementProjectView() {
    ...
    return `
      ...
      <section class="data-panel settlement-register ui-v2-payroll-panel ui-v2-payroll-register"><div class="table-toolbar ui-v2-payroll-register__toolbar"><div class="table-toolbar__search ui-v2-filter-bar__search">${icon('search')}<input id="rentalSettlementSearch" type="search" value="${escapeHtml(state.rentalSettlementSearch)}" placeholder="Search worker, trade or supplier…"></div><label class="timesheet-filter"><span>Project</span><select id="rentalSettlementProject" class="compact-select ui-v2-select ui-v2-payroll-compact-select">${state.projects.map(p=>`<option value="${escapeHtml(p.id)}" ${p.id===project.id?'selected':''}>${escapeHtml(p.name)}</option>`).join('')}</select></label><label class="timesheet-filter"><span>Supplier</span><select id="rentalSettlementSupplier" class="compact-select ui-v2-select ui-v2-payroll-compact-select"><option value="All suppliers">All suppliers</option>${suppliers.map(s=>`<option value="${escapeHtml(s.id)}" ${s.id===state.rentalSettlementSupplier?'selected':''}>${escapeHtml(s.name)}</option>`).join('')}</select></label><button class="btn btn--ghost" data-settlement-reset>Reset</button></div>
        <div class="table-meta ui-v2-payroll-table-meta"><span><strong>${rows.length}</strong> worker snapshot${rows.length===1?'':'s'}</span><span>Locked timesheet + effective commercial snapshot + Approved rental adjustments</span></div>
  ...
  function openSupplierEditDrawer(supplierId) {
    ...
    drawerBody.innerHTML = formSections([
      ['Supplier master', 'Update the managed supplier record. Existing worker and financial history remains linked to the same supplier ID.', [
        ...
        namedSelectFieldValue('Status', 'supplier-status', ['Active','Inactive'], supplier.status || 'Active'),
        ...
        try {
          ...
          const payload = await appApi(`/api/rental/workers/${encodeURIComponent(worker.id)}/`, {
            method:'PATCH', body:{
              ...
              supplier_id:worker.supplierId, status:get('rental-action-master-status') || worker.masterStatus || 'Active',
      ...
      try {
        ...
        const payload = await appApi(editing ? `/api/rental/suppliers/${current.id}/` : '/api/rental/suppliers/', {
          ...
          body:{
            ...
            status:get('supplier-status') || 'Active',
