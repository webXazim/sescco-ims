# Accounts Rental Supervisor Migration Hotfix — 1.0.114
...
1.0.114 corrects that historical lookup to `apps.get_model("core", "Company")` and routes all migration-time ORM operations through `schema_editor.connection.alias`. The migration remains named `accounts.0008_rental_supervisor_profile`; do not fake it.

PostgreSQL migrations are transactional by default, so the failed attempt should have rolled back. Deploy 1.0.114 and rerun the normal migration pipeline.
