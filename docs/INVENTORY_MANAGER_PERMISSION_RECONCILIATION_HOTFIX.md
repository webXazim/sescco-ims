# Inventory Manager Permission Reconciliation Hotfix — 1.0.115
...
1.0.115 adds forward data migration `accounts.0014_inventory_manager_import_permission`. It adds exactly `inventory.import.execute` to active system Inventory Manager profiles when missing. It does not modify Storekeeper, custom profiles, memberships, scope rows, Inventory quantities, Payroll formulas, or the permission catalog.
